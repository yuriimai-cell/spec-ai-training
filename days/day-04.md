...
Git では **ブランチ（branch）** という仕組みを使って作業する。

```
main（本体）─────────────────────────── ← 常に安定した状態を保つ
      └── training/day-04-tool-comparison ← 自分の作業はここで行う
```

- **main ブランチ** = 本体。完成したものだけが入っている
- **作業ブランチ** = 自分の作業場所。main から分岐して作る
- 作業が終わったら **PR（Pull Request）** を作って、作業ブランチの内容を main に合流させる

**なぜこうするのか?**
- main を直接いじると、途中の状態が他の人に見えてしまう
- ブランチを使えば、作業中の変更は自分だけの場所に隔離される
- PR でレビューを受けてから合流するので、品質を保てる

**毎日のワークフロー:**

`ブランチ作成` → `ファイル作成・編集` → `commit（保存）` → `push（アップロード）` → `PR作成（レビュー依頼）`
